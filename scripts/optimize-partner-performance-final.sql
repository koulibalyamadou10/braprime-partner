-- Script d'optimisation des performances pour les partenaires (VERSION FINALE CORRIGÉE)
...
-- Index sur les notifications par user_id et date
...
-- 2. FONCTION RPC POUR LES STATISTIQUES PARTENAIRES (corrigée)
...
-- 3. FONCTION RPC POUR LES DONNÉES DE REVENUS (corrigée)
...
-- 4. FONCTION RPC POUR LES ARTICLES POPULAIRES (corrigée)
...
-- 5. FONCTION POUR OPTIMISER LES REQUÊTES DE PAGINATION (corrigée)
    ...
    COALESCE(
      json_agg(
        json_build_object(
          'id', o.id,
          'user_id', o.user_id,
          'customer_name', COALESCE(up.name, 'Client'),
          'customer_phone', COALESCE(up.phone_number, ''),
          'items', o.items,
          'status', o.status,
          'total', o.total,
...
-- 6. FONCTION POUR RÉCUPÉRER LES NOTIFICATIONS D'UN PARTENAIRE
...
-- 7. FONCTION POUR LES NOTIFICATIONS EN TEMPS RÉEL
...
-- 8. FONCTION POUR RÉCUPÉRER LES LIVREURS D'UN BUSINESS (ajoutée)

CREATE OR REPLACE FUNCTION get_partner_drivers_paginated(
  business_id_param INTEGER,
...
BEGIN
  ...
  SELECT COUNT(*) INTO total_count_val
  FROM drivers d
  WHERE d.business_id = business_id_param
    AND (is_active_filter IS NULL OR d.is_active = is_active_filter);
    ...
    COALESCE(
      json_agg(
        json_build_object(
          ...
          'total_earnings', COALESCE(d.total_earnings, 0),
...
COMMENT ON FUNCTION get_partner_drivers_paginated(INTEGER, INTEGER, INTEGER, BOOLEAN) IS 'Récupère les livreurs d''un partenaire avec pagination';
...
GRANT EXECUTE ON FUNCTION get_partner_drivers_paginated(INTEGER, INTEGER, INTEGER, BOOLEAN) TO authenticated;
...
BEGIN
  ...
  RAISE NOTICE 'Fonctions RPC créées: 6';
  RAISE NOTICE 'Triggers créés: 1';
  RAISE NOTICE 'Corrections: GROUP BY clauses et relations corrigées';
  RAISE NOTICE 'Relations: user_profiles et orders correctement jointes';
END $$; 